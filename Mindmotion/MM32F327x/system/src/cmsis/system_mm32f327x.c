...
static void SetSysClockToAnyXX(void)
{
    ...
    tempreg &= ~FLASH_ACR_LATENCY;
    temp = (SystemCoreClock - 1) / 24000000;
    tempreg |= (temp & FLASH_ACR_LATENCY);
    FLASH->ACR = tempreg;

    tempreg = RCC->CFGR;
    tempreg &= (~RCC_CFGR_HPRE) & ( ~RCC_CFGR_PPRE1) & (~RCC_CFGR_PPRE2);

    //HCLK = AHB = FCLK = SYSCLK divided by 4
    tempreg |= (u32)RCC_CFGR_HPRE_DIV4;

    //PCLK2 = APB2 = HCLK divided by 1, APB2 is high APB CLK
    tempreg |= (u32)RCC_CFGR_PPRE2_DIV1;

    //PCLK1 = APB1 = HCLK divided by 1, APB1 is low APB CLK
    tempreg |= (u32)RCC_CFGR_PPRE1_DIV1;
